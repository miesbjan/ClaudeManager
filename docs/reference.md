...
Everything the [README](../README.md) leaves out. The README is the short way in; this
file is the long one, kept for when a detail actually matters - what a readout means,
what the app does on its own, what the security model rests on, and where the code for
any of it lives.
...
- **The log.** Windows gives a packaged application no console, so the events nobody
  can see are written to `log.txt` in `%APPDATA%\claude-manager`: a window that died and
...
- **Terminal font.** `Ctrl+=` and `Ctrl+-` change the size, between 8 and 28, and
  ...
  `%APPDATA%/claude-manager/settings.json`. That file is only ever read, never
...
- **Session.** The list of open files and the active tab are stored in
  `%APPDATA%/claude-manager/state.json`, together with the window geometry, and restored
