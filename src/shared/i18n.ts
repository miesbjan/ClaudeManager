...
const en = {
  ...
  'toolbar.web': 'Web',
  'toolbar.web.title': 'Document or dev server (Alt+W)',
  ...
  'palette.forgotten': '{name} will not be offered here again.',
  'palette.dropped': '{name} is closed and will not be offered here again.',
  'palette.drop': 'Close {name} and stop offering it here',
  'pane.noSuchFile': 'There is no file {index} open here - {count} open in this tab.',
  'prompt.noShell': 'No shell would start here, so nothing was sent and the prompt is still yours.',
  'run.noShell': 'No shell would start here, so there was nothing to run the command in.',
  'log.written': 'Wrote what {count} shell(s) have printed, next to the log in %APPDATA%.',
  'log.nothing': 'No shell is running, so there was nothing to write down.',
  'web.reload': 'Reload',
...
const cs: Record<StringKey, string> = {
  ...
  'toolbar.web': 'Web',
  'toolbar.web.title': 'Dokument, nebo dev server (Alt+W)',
  ...
  'palette.forgotten': '{name} už se tu nabízet nebude.',
  'palette.dropped': '{name} je zavřený a nabízet se tu už nebude.',
  'palette.drop': 'Zavřít {name} a přestat ho tady nabízet',
  'pane.noSuchFile': 'Soubor {index} tady otevřený není - v tomhle tabu jsou {count}.',
  'prompt.noShell': 'Tady se nepodařilo nastartovat shell, takže se nic neposlalo a prompt ti zůstal.',
  'run.noShell': 'Tady se nepodařilo nastartovat shell, takže nebylo v čem příkaz spustit.',
  'log.written': 'Zapsáno, co vypsalo {count} shell(ů), vedle logu v %APPDATA%.',
  'log.nothing': 'Neběží žádný shell, takže nebylo co zapsat.',
